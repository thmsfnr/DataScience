...
plt.plot(df['ds'],df['y'])
plt.xlabel('Année')
plt.ylabel('Production d\'électricité en MWh pour le hydrolique')
plt.title('Production d\'électricité en MWh pour le hydrolique en fonction des années')
plt.grid(False)
plt.show()
...
plt.xlabel('Année')
plt.ylabel('Production d\'électricité en MWh pour le hydrolique')
plt.title('Prédiction de la production d\'électricité en MWh pour le hydrolique en fonction des années')
plt.show()
...
print(forecast)
afficheGraphePrediction(df,forecast,'Analyse de la prédiction de la production d\'électricité en MWh pour le hydrolique en fonction des années')
...
#90 % entrainement et 10 % test ( 365 * 2 ans =  jours) (de 2008 à 2010) donc je veux tester sur les 10 ans restant soit 365*10= 3650 days
df_cv = cross_validation(m, initial = '730 days', period='365 days', horizon = '3650 days')
print("CROSS VALIDATION")
print(df_cv)
afficheGraphePrediction(df_cv,df_cv,'Analyse de la cross validation de la production d\'électricité en MWh pour le hydrolique en fonction des années')
